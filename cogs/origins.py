...
class Origins(commands.Cog):
	...
	async def tags(self, ctx, *, tag_type):
		match tag_type.lower():
			...
			case ('block' | 'blocks'):
				await bot.tryreply(ctx, 'A block tag is a JSON file used for grouping blocks. It\'s stored inside the `data\\<namespace>\\tags\\blocks` folder, where `<namespace>` is the folder you\'re using to store your advancements, functions, loot tables, powers, origins, etc.\n\nHere\'s an example of a block tag, named `leaves.json`, stored inside the `data\\minecraft\\tags\\blocks` folder:\n```json\n{\n    "replace": false,\n    "values": [\n        "minecraft:jungle_leaves",\n        "minecraft:oak_leaves",\n        "minecraft:spruce_leaves",\n        "minecraft:dark_oak_leaves",\n        "minecraft:acacia_leaves",\n        "minecraft:birch_leaves"\n    ]\n}\n```\nYou can then reference the `minecraft:leaves` block tag inside a block condition, like `origins:in_tag`, like so:\n```json\n"block_condition": {\n    "type": "origins:in_tag",\n    "tag": "minecraft:leaves"\n}\n```', reply=True)
			case ('item' | 'items'):
